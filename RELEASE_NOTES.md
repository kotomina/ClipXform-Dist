...
### 新機能 (Features)
- **Vertex AI の Gemini 3.5 Flash 候補追加**: Vertex AI / Gemini Enterprise Agent Platform で利用するモデル ID として `gemini-3.5-flash` を設定画面の Gemini モデル候補に追加しました。
...
### 新機能 (Features)
- **クリップボード・マクロ (v1)**: JavaScript ベースのマクロ機能を追加しました。クリップボードのテキストを入力に、既存変換、AI 呼び出し、ファイル書き込み、ブラウザ起動、外部コマンド実行、HTTP 通信を 1 本のスクリプトとして扱えます。
- **Host API リファレンス同梱**: マクロ画面から参照できる host API リファレンスをアプリ内に同梱しました。

### 改善 (Improved)
- **AI マクロ生成の安定化**: AI 生成プロンプトを JavaScript 前提に刷新し、`async` / `await` や `async IIFE` を返しても同期 host API 形式へ正規化するよう改善しました。
- **入力チェックと保存確認の強化**: 保存前に JavaScript 構文・必要 capability を検証し、AI 利用、ファイル書き込み、外部コマンド実行、通信を含む場合は使用 host API とリスク理由を明示して確認するようにしました。
- **特権マクロ運用の明確化**: `process.run` / `net.fetch` の allowlist、ワイルドカード、`regex:` 正規表現、都度確認フローを整備しました。

## v0.17.15 (2026-05-10)

### 修正 (Fixed)
- **Vertex AI の Gemini フォールバック安定化**: Vertex AI API key 利用時は Gemini の一覧 API 呼び出しをスキップし、設定画面表示時の 403 や遅延待ち、不要ログを回避するよう修正しました。
- **Ollama 設定画面の切り替え回帰修正**: 保存済み Base URL がある環境で設定画面内から Ollama に切り替えた直後にもモデル候補を再取得し、ドロップダウンが空のままになる問題を修正しました。
- **画面遷移時のメモリリーク修正**: 設定・履歴・定型文・変換画面で `DotNetObjectReference` を明示破棄し、繰り返し遷移時に JS 連携用参照が残る問題を修正しました。
...
### 追加 (Added)
- **Gemini の Vertex AI API key 対応**: Gemini 設定で `Gemini Developer API` と `Vertex AI` の接続先を切り替えられるようにし、Vertex AI の API key でも画像OCRやテキスト生成を利用できるようにしました。

### 改善 (Improved)
- **Gemini セットアップ導線の明確化**: 設定画面とセットアップウィザードで、接続先に応じて `Google AI Studio` と `Vertex AI` の適切なキー取得先を案内するよう改善しました。
